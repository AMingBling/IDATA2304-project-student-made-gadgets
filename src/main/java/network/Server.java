        ...
        if (initialMessage != null) {
          ...
          if (trimmed.equals("CONTROL_PANEL_CONNECTED")) {
            ...
            if (sensorNodes.containsKey(nodeId)) {
              out.println("NODE_ID_REJECTED");
              System.out.println(
                  "Rejected duplicate nodeId '" + nodeId + "' from " + socket.getInetAddress());
              socket.close();
              return;
            } else {
              ...
              System.out.println(
                  "entity.Sensor connected: " + socket.getInetAddress() + " (" + nodeId + ")");
            }
            ...
            try {
              ...
              if (obj != null && obj.has("messageType") && "REGISTER_CONTROL_PANEL".equals(
                ...
                String cpId = obj.has("controlPanelId") ? obj.get("controlPanelId").getAsString()
                    : "<unknown>";
                System.out.println(
                    "Control panel registered: " + socket.getInetAddress() + " (id=" + cpId + ")");
                handledInitial = true;
              }
                ...
                if ("ACTUATOR_COMMAND".equals(mt) || "TARGET_UPDATE".equals(mt)) {
                  ...
                  if (obj.has("nodeID") && !obj.get("nodeID").isJsonNull()) {
                    ...
                    if ("SUBSCRIBE_NODE".equals(mt)) {
                      subs.add(targetNode);
                      System.out.println(
                          "Control panel " + socket.getInetAddress() + " subscribed to "
                              + targetNode);
                      ...
                      System.out.println(
                          "Control panel " + socket.getInetAddress() + " unsubscribed from "
                              + targetNode);
                  ...
                  if (obj.has("nodeID") && !obj.get("nodeID").isJsonNull()) {
                    ...
                    if (last != null) {
                      ...
                      System.out.println("Served last-known state for " + targetNode + " to "
                      ...
                      if (nodeSocket != null && !nodeSocket.isClosed()) {
                        try {
                          ...
                          System.out.println(
                              "Error forwarding REQUEST_STATE to node: " + e.getMessage());
                        }
                      } else {
                        System.out.println(
                            "Target node not connected for REQUEST_NODE: " + targetNode);
                      }
                    }
                  }
                } else {
